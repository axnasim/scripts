...
if command -v az &>/dev/null; then
  ...
  if command -v jq &>/dev/null; then
    version=$(az version 2>/dev/null | jq -r '.["azure-cli"]' 2>/dev/null)
  fi

  # Fallback: use grep if jq failed or isn't installed
  if [[ -z "$version" || "$version" == "null" || "$version" == "unknown" ]]; then
    version=$(az version 2>/dev/null | grep -oP '"azure-cli":\s*"\K[^"]+')
  fi

  echo "✅ Azure CLI is already installed. Version: ${version:-unknown}"
  exit 0
fi
